...
/**
 * IFS Invoice Tax Request XML To Avalara
 *
 * IMPORTANT !!!
 * Do not place this class inside a package. It should be
 * in the default package.
 */
public class IFSInvoiceTaxRequestXMLToAvalara implements Transformer {
   ...
   public String transform(String sIn) throws IfsException {
      ...
      try {
         ...
         if (rootNode.getNodeType() == Node.ELEMENT_NODE) {
            user = getValueByTagName("USER_ID", (Element) rootNode);
         ...
         NodeList orderRequestList = doc.getElementsByTagName("GET_TAX_REQUEST");
         for (int i = 0; i < orderRequestList.getLength(); i++) {
            ...
            if (orderRequestNode.getNodeType() == 1) {
               ...
               getTaxRequestElem.addChildElement("CompanyCode", SER_NS_PREFIX).addTextNode(getValueByTagName("COMPANY_CODE", refOrderRequestElem));
         ...
         Logger.getLogger(IFSInvoiceTaxRequestXMLToAvalara.class.getName()).log(Level.SEVERE, null, ex);
      } 
