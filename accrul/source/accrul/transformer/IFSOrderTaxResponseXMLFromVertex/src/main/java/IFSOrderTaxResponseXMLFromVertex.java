...
/**
 * IFS Order Tax Response XML From Vertex
 *
 * IMPORTANT !!!
 * Do not place this class inside a package. It should be
 * in the default package.
 */
public class IFSOrderTaxResponseXMLFromVertex implements Transformer {
   ...
   public String transform(String sIn) throws IfsException {
      ...
      try {
         ...
         NodeList msgLinesa = doc.getElementsByTagName("QuotationResponse");
         sOut.append("<TAX_ORDER_RESPONSE>\n");
         sOut.append("<ORDER_RESPONSES>\n");
         sOut.append("<ORDER_RESPONSE>\n");
            ...
            if (msgLinea.getNodeType() == Node.ELEMENT_NODE) {
               ...
               for (int j = 0; j < msgLinesb.getLength(); j++) {
                  ...
                  if (msgLineb.getNodeType() == Node.ELEMENT_NODE) {
                     ...
                     sOut.append("<PRODUCT>").append(encode(getValue(refElementa, "Product", j))).append("</PRODUCT>\n");
                     sOut.append("<PRODUCT_CLASS>").append("").append("</PRODUCT_CLASS>\n");
