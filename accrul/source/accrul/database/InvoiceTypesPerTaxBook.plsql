-----------------------------------------------------------------------------
--
--  Logical unit: InvoiceTypesPerTaxBook
--  Component:    ACCRUL
--
...
PROCEDURE Check_Insert___ (
   newrec_ IN OUT invoice_types_per_tax_book_tab%ROWTYPE,
...
-- Validate_Insert___
--    Same invoice type should not be used in other tax books with same direction
PROCEDURE Validate_Insert___ (
   newrec_ IN invoice_types_per_tax_book_tab%ROWTYPE )
IS
...
BEGIN
   ...
   IF get_tax_book%FOUND THEN
      CLOSE get_tax_book;
      Error_SYS.Appl_General(lu_name_, 'INVTYPEALREADYUSED: Invoice Type :P1 is used in another Tax Book having same Tax Direction.', newrec_.type_id);
   END IF;
   CLOSE get_tax_book;
