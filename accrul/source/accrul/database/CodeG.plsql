...
--  ------  ------  ---------------------------------------------------------
--  960404  MIJO  Created.
--  980708  Kanchi Modified to accomadate codepart 'G' functionality  through
--                 this LU.
...
--                 directed to this CodeG LU:s copied functions/procedures.
--  980916  Ruwan  Removed the Get_Description overloaded function(Bug ID:4973)
--  980921  Bren   Master Slave Connection
--                 Added Send_Code_G_Info___, Send_Code_G_Info_Delete___,
--                 Send_Code_G_Info_Modify___, Receive_Code_G_Info___
...
--  010619  LiSv   For new Create Company concept added new view code_g_ect and code_g_pct.
...
--  021002  Nimalk Removed usage of the view Company_Finance_Auth in CODE_G,CODE_G_PCT view
...
--  100304  Nsillk EAFH-2369 , Modified method Copy
...
--  131205  MEALLK PBFI-1985, refactored code according to new standards
...
pc_code_part_             CONSTANT VARCHAR2(1)  := 'G';
...
BEGIN
   ...
   IF ( run_crecomp_ ) THEN
      ...
      IF records_exists_ AND code_part_function_db_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeG','FUNCUSED: Can not insert values for Code part :P1 since the code '||
      ...
      ELSIF NOT records_exists_ THEN
         ...
         Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_G_API', 'CreatedSuccessfully', msg_);
      ELSE
         super(crecomp_rec_);
      END IF;
   END IF;
EXCEPTION
   WHEN OTHERS THEN
      msg_ := SQLERRM;
      Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_G_API', 'Error', msg_);
      Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_G_API', 'CreatedWithErrors');  
END Import___;
...
BEGIN
   ...
   IF ( run_crecomp_ ) THEN
      ...
      IF records_exists_ AND code_part_function_db1_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeG','FUNCUSED1: Can not copy Code part :P1 since the code part in the source company :P2'||
      ...
      ELSIF  records_exists_ AND code_part_function_db2_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeG','FUNCUSED2: Can not copy Code part :P1 since the code part in company :P2'||
                                  ' has code part values',
...
BEGIN
   ...
   END LOOP;
   IF records_exists_ AND code_part_function_db_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
      Error_SYS.Record_General('CodeG','FUNCUSED3: Can not export Code part :P1 since the code part in the company :P2'||
...
BEGIN
   ...
   Client_SYS.Add_To_Attr( 'CODE_PART','G', attr_ );
...
BEGIN
   ...
   -- if CODE_G has been unpacked then do not get value from CODE_PART_VALUE since 
   -- it is then passed in CODE_G.
...
BEGIN
   ...
   IF ((INSTR( newrec_.code_part_value,'&') > 0) OR (INSTR( newrec_.code_part_value,'^') > 0) OR (INSTR( newrec_.code_part_value,'''') > 0)) THEN
      Error_SYS.Record_General('CodeG', 'INVCHARACTORS: You have entered an invalid character in this field');
   END IF;
...
BEGIN
   ...
   Error_SYS.Record_General(lu_name_,'NOTEXTCODEF: The :P2 code part value :P1 does not exists.', code_g_ ,code_part_name_ );
   super(company_, code_g_);
   --Add post-processing code here
...
BEGIN
   log_key_ := code_g_;   
   Accounting_Code_Part_Value_API.Check_Copy_Code_Part_Function(target_company_, 'G');
   source_rec_ := Get_Object_By_Keys___(source_company_, code_g_);
   target_rec_ := source_rec_;
   target_rec_.company := target_company_;
   App_Context_Sys.Set_Value(lu_name_||'.copy_to_company_', 'TRUE');
   old_target_rec_ := Get_Object_By_Keys___(target_company_, code_g_);
   ...
   ELSIF (source_rec_.rowversion IS NULL AND old_target_rec_.rowversion IS NULL) THEN
      ...
      Raise_Record_Not_Exist___(target_company_, code_g_);
...
IS
   result_      VARCHAR2(6);
BEGIN
   Validate_Code_Part(result_,company_, code_g_,date_);
   RETURN result_;
...
IS
BEGIN
   Accounting_Code_Part_Value_API.Check_Delete_Allowed2 (company_,code_g_,'G');
...
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Exist_Code_Part_Value (company_,'G');
...
FUNCTION Code_Part_Value_Exist (
   company_   IN VARCHAR2,
   code_g_    IN VARCHAR2 ) RETURN VARCHAR2
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Code_Part_Value_Exist (company_,'G',code_g_);
...
IS
   dummy_          BOOLEAN;
BEGIN
   dummy_ := Validate_Code_Part(company_, code_g_, date_ );
   IF ( dummy_ ) THEN
      result_ := 'TRUE' ;
   ELSE
      result_ := 'FALSE' ;
...
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Validate_Code_Part (company_,'G',code_g_,date_);
END Validate_Code_Part;
...
IS
BEGIN
   description_ := CODE_G_API.Get_Description(company_, code_part_val_);
...
IS
   ...
   CURSOR get_desc IS
      ...
      AND   code_part = 'G'
      AND   code_part_value = code_g_;
BEGIN
   desc_ := substr(Text_Field_Translation_API.Get_Text ( company_, 'CODEG',code_g_, lang_code_),1, 100);
...
IS   
BEGIN
   description_ := Get_Description(company_, code_g_, lang_code_);   
END Get_Description;
...
BEGIN
   new_attr_ := attr_;
   Client_SYS.Add_To_Attr('CODE_PART', 'G', new_attr_);
   code_part_value_list_ := Client_SYS.Cut_Item_Value('CODE_G_LIST',  new_attr_);
