...
--  ------  ------  ---------------------------------------------------------
--  960404  MIJO  Created.
--  980708  Kanchi Modified to accomadate codepart 'B' functionality  through
--                 this LU.
...
--                 directed to this CodeB LU:s copied functions/procedures.
--  980916  Ruwan  Removed the Get_Description overloaded function(Bug ID:4973)
--  990707  UMA    Modified with respect to new template
--  990730  SaCh   Modified with respect to new templates.
...
--  001204  LiSv   For new Create Company concept added new view code_b_ect and code_b_pct.
...
--  020303  Shsalk Call Id 76915 Corrected.
--  021002  Nimalk Removed usage of the view Company_Finance_Auth in CODE_B,CODE_B_PCT view
...
--  030102  dagalk Merge SP3 bug corrections.
...
--  040723  Gawilk FIPR307A: Added view CODE_B_LOV
--  050202  nsillk Merged Bug 42667.
--  050321  upunlk  LCS Merge 47814 - Changed the valid_until date of the method Copy().
--  051013  Shsalk Removed the view CODE_B_LOV.
--  051021  Nsillk LCS Merge(52983) Recorrected.
...
--  100302  Nsillk EAFH-2150, Modified the Copy method so the dates will be consistent with the new company creatin concept
...
--  110523  DIFELK EASTONE-21144, modified Copy___
...
--  131205  Janblk PBFI-1980, Refactoring and Split  CodeB.entity
...
pc_code_part_             CONSTANT VARCHAR2(1) := 'B';
...
BEGIN
   ...
   Error_SYS.Record_General('CodeB','NOTEXTCODEB: The :P2 code part value :P1 does not exists.', code_b_ ,code_part_name_ );
   super(company_, code_b_);
...
BEGIN
   ...
   IF ( run_crecomp_ ) THEN
      ...
      IF records_exists_ AND code_part_function_db_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeB','FUNCUSED: Can not insert values for Code part :P1 since the code '||
      ...
      ELSIF NOT records_exists_ THEN
         ...
         Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_B_API', 'CreatedSuccessfully', msg_);
      ELSE
         super(crecomp_rec_);
      END IF; 
   END IF;
EXCEPTION
   WHEN OTHERS THEN
      msg_ := SQLERRM;
      Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_B_API', 'Error', msg_);
      Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_B_API', 'CreatedWithErrors');
END Import___;
...
BEGIN
   ...
   IF ( run_crecomp_ ) THEN
      ...
      IF records_exists_ AND code_part_function_db1_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeB','FUNCUSED1: Can not copy Code part :P1 since the code part in the source company :P2'||
      ...
      ELSIF  records_exists_ AND code_part_function_db2_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeB','FUNCUSED2: Can not copy Code part :P1 since the code part in company :P2'||
                                  ' has code part values',
...
BEGIN
   ...
   END LOOP;
   IF records_exists_ AND code_part_function_db_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
      Error_SYS.Record_General('CodeB','FUNCUSED3: Can not export Code part :P1 since the code part in the company :P2'||
...
BEGIN
   ...
   -- if CODE_B has been unpacked then do not get value from CODE_PART_VALUE since 
   -- it is then passed in CODE_B.
...
BEGIN
   ...
   Client_SYS.Add_To_Attr( 'CODE_PART','B', attr_ );
...
BEGIN
   ...
   IF ((INSTR( newrec_.code_part_value,'&') > 0) OR (INSTR( newrec_.code_part_value,'^') > 0) OR (INSTR( newrec_.code_part_value,'''') > 0)) THEN
      Error_SYS.Record_General('CodeB', 'INVCHARACTORS: You have entered an invalid character in this field');
   END IF;
...
BEGIN
   log_key_ := code_b_;
   Accounting_Code_Part_Value_API.Check_Copy_Code_Part_Function(target_company_, 'B');
   source_rec_ := Get_Object_By_Keys___(source_company_, code_b_);
   target_rec_ := source_rec_;
   target_rec_.company := target_company_;
   App_Context_Sys.Set_Value(lu_name_||'.copy_to_company_', 'TRUE');
   old_target_rec_ := Get_Object_By_Keys___(target_company_, code_b_);
   ...
   ELSIF (source_rec_.rowversion IS NULL AND old_target_rec_.rowversion IS NULL) THEN
      ...
      Raise_Record_Not_Exist___(target_company_, code_b_);
...
IS
   result_      VARCHAR2(6);
BEGIN
   Validate_Code_Part(result_,company_, code_b_,date_);
   RETURN result_;
...
IS
BEGIN
   Accounting_Code_Part_Value_API.Check_Delete_Allowed2 (company_,code_b_,'B');
...
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Exist_Code_Part_Value (company_,'B');
...
FUNCTION Code_Part_Value_Exist (
   company_   IN VARCHAR2,
   code_b_    IN VARCHAR2 ) RETURN VARCHAR2
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Code_Part_Value_Exist(company_, 'B', code_b_);
...
IS
   dummy_       BOOLEAN;
BEGIN
   dummy_ := Validate_Code_Part(company_, code_b_, date_ );
   IF (dummy_) THEN
      result_ := 'TRUE' ;
   ELSE
      result_ := 'FALSE' ;
...
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Validate_Code_Part (company_,'B',code_b_,date_);
END Validate_Code_Part;
...
IS
BEGIN
   description_ := Get_Description(company_, code_part_val_);
...
IS
   ...
   CURSOR get_desc IS
      ...
      AND   code_part = 'B'
      AND   code_part_value = code_b_;
BEGIN
   desc_ := substr(Text_Field_Translation_API.Get_Text ( company_, 'CODEB',code_b_, lang_code_),1, 100);
...
IS   
BEGIN
   description_ := Get_Description(company_, code_b_, lang_code_);   
END Get_Description;
...
BEGIN
   new_attr_ := attr_;
   Client_SYS.Add_To_Attr('CODE_PART', 'B', new_attr_);
   code_part_value_list_ := Client_SYS.Cut_Item_Value('CODE_B_LIST',  new_attr_);
