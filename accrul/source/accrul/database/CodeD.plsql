...
--  ------  ------  ---------------------------------------------------------
--  960404  MIJO   Created.
--  980708  Kanchi Modified to accomadate codepart 'D' functionality  through
--                 this LU.
...
--                 directed to this CodeD LU:s copied functions/procedures.
--  980916  Ruwan  Removed the Get_Description overloaded function(Bug ID:4973)
--  980921  Bren   Master Slave Connection
--                 Added Send_Code_D_Info___, Send_Code_D_Info_Delete___,
--                 Send_Code_D_Info_Modify___, Send_Code_D_Info___
--  990708  Uma    Modified with respect to new template
...
--  010619  LiSv   For new Create Company concept added new view code_d_ect and code_d_pct.
...
--  021002  Nimalk Removed usage of the view Company_Finance_Auth in CODE_D,CODE_D_PCT view
...
--  040723  Gawilk FIPR307A: Added view CODE_D_LOV
--  050202  nsillk Merged Bug 42667.
--  050321  upunlk  LCS Merge 47814 - Changed the valid_until date of the method Copy().
--  051013  Shsalk Removed the view CODE_D_LOV.
--  051024  Nsillk 52983 Merge Recorrected.
...
--  100302  Nsillk EAFH-2152 , Modified method Copy__
...
--  131205  Janblk PBFI-1982: Refactoring and Split  CodeD.entit
...
pc_code_part_             CONSTANT VARCHAR2(1)  := 'D';
...
BEGIN
   ...
   Error_SYS.Record_General('CodeD','NOTEXTCODEB: The :P2 code part value :P1 does not exists.', code_d_ ,code_part_name_ );
   super(company_, code_d_);
...
BEGIN
   ...
   IF ( run_crecomp_ ) THEN
      ...
      IF records_exists_ AND code_part_function_db_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeD','FUNCUSED: Can not insert values for Code part :P1 since the code '||
      ...
      ELSIF NOT records_exists_ THEN
         ...
         Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_D_API', 'CreatedSuccessfully', msg_);
      ELSE        
         super(crecomp_rec_);
      END IF;
   END IF;
EXCEPTION
   WHEN OTHERS THEN
      msg_ := SQLERRM;
      Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_D_API', 'Error', msg_);
      Enterp_Comp_Connect_V170_API.Log_Logging(crecomp_rec_.company, module_, 'CODE_D_API', 'CreatedWithErrors');
   END Import___;
...
BEGIN
   ...
   IF ( run_crecomp_ ) THEN
      ...
      IF records_exists_ AND code_part_function_db1_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeD','FUNCUSED1: Can not copy Code part :P1 since the code part in the source company :P2'||
      ...
      ELSIF  records_exists_ AND code_part_function_db2_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
         Error_SYS.Record_General('CodeD','FUNCUSED2: Can not copy Code part :P1 since the code part in company :P2'||
                                  ' has code part values',
...
BEGIN
   ...
   END LOOP;
   IF records_exists_ AND code_part_function_db_ NOT IN ('CURR', 'ELIM', 'INTERN', 'NOFUNC') THEN
      Error_SYS.Record_General('CodeD','FUNCUSED3: Can not export Code part :P1 since the code part in the company :P2'||
...
BEGIN
   ...
   -- if CODE_D has been unpacked then do not get value from CODE_PART_VALUE since 
   -- it is then passed in CODE_D.
...
BEGIN
   ...
   Client_SYS.Add_To_Attr( 'CODE_PART','D', attr_ );
...
BEGIN
   ...
   IF ((INSTR( newrec_.code_part_value,'&') > 0) OR (INSTR( newrec_.code_part_value,'^') > 0) OR (INSTR( newrec_.code_part_value,'''') > 0)) THEN
      Error_SYS.Record_General('CodeD', 'INVCHARACTORS: You have entered an invalid character in this field');
   END IF;
...
BEGIN
   log_key_ := code_d_;
   Accounting_Code_Part_Value_API.Check_Copy_Code_Part_Function(target_company_, 'D');
   source_rec_ := Get_Object_By_Keys___(source_company_, code_d_);
   target_rec_ := source_rec_;
   target_rec_.company := target_company_;
   App_Context_Sys.Set_Value(lu_name_||'.copy_to_company_', 'TRUE');
   
   old_target_rec_ := Get_Object_By_Keys___(target_company_, code_d_);
   ...
   ELSIF (source_rec_.rowversion IS NULL AND old_target_rec_.rowversion IS NULL) THEN
      ...
      Raise_Record_Not_Exist___(target_company_, code_d_);
...
IS
   result_      VARCHAR2(6);
BEGIN
   Validate_Code_Part(result_,company_, code_d_,date_);
   RETURN result_;
...
IS
BEGIN
   Accounting_Code_Part_Value_API.Check_Delete_Allowed2 (company_,code_d_,'D');
...
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Exist_Code_Part_Value (company_,'D');
...
FUNCTION Code_Part_Value_Exist (
   company_   IN VARCHAR2,
   code_d_    IN VARCHAR2 ) RETURN VARCHAR2
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Code_Part_Value_Exist (company_,'D',code_d_);
...
IS
   dummy_      BOOLEAN;
BEGIN
   dummy_ := Validate_Code_Part(company_, code_d_, date_ );
   IF ( dummy_ ) THEN
      result_ := 'TRUE' ;
   ELSE
      result_ := 'FALSE' ;
...
IS
BEGIN
   RETURN Accounting_Code_Part_Value_API.Validate_Code_Part (company_,'D',code_d_,date_);
END Validate_Code_Part;
...
IS
   ...
   CURSOR get_desc IS
      ...
      AND   code_part = 'D'
      AND   code_part_value = code_d_;
BEGIN
   desc_ := substr(Text_Field_Translation_API.Get_Text ( company_, 'CODED',code_d_, lang_code_),1, 100);
...
IS   
BEGIN
   description_ := Get_Description(company_, code_d_, lang_code_);   
END Get_Description;
...
BEGIN
   new_attr_ := attr_;
   Client_SYS.Add_To_Attr('CODE_PART', 'D', new_attr_);
   code_part_value_list_ := Client_SYS.Cut_Item_Value('CODE_D_LIST',  new_attr_);
