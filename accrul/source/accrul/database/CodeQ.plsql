...
BEGIN      
   super(attr_);
   Accounting_Code_Part_Value_API.Add_Default_Values_Additional(attr_, 'Q');
...
BEGIN
   IF ((INSTR( newrec_.code_part_value,'&') > 0) OR (INSTR( newrec_.code_part_value,'^') > 0) OR (INSTR( newrec_.code_part_value,'''') > 0)) THEN
      Error_SYS.Record_General('CodeQ', 'INVCHARACTORS: You have entered an invalid character in this field');
   END IF;
