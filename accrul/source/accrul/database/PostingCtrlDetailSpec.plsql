...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  050601  reanpl  Created for FIAD376 - Actual Costing
--  060622  iswalk  removed allowed_default_.
--  070321  Shsalk  LCS Merge 63645,Corrected.Added new method Account_Exist
...
--  151007  chiblk  STRFI-200, New__ changed to New___ in Insert_Posting_Ctrl_Det_Spec
--  190419  Nudilk  Bug 147695, Added Get_Crtl_Type_Value_Desc.
...
PROCEDURE Validate_Row___ (
   newrec_ IN POSTING_CTRL_DETAIL_SPEC_TAB%ROWTYPE )
IS
   ...
   IF (newrec_.code_part = 'A') THEN
      IF ledg_flag_ = 'Y' THEN
         IF (NOT ACCOUNTING_CODE_PART_A_API.Is_Ledger_Account( newrec_.company, newrec_.code_part_value )) THEN
            Error_SYS.Record_General(lu_name_, 'SPECNOLEDGACCNT: :P1 is no ledger account and not valid for posting type :P2', newrec_.code_part_value,newrec_.posting_type );
         END IF;
      ELSIF tax_flag_ = 'Y' THEN
         IF (NOT Account_API.Is_Tax_Account( newrec_.company, newrec_.code_part_value )) THEN
            Error_SYS.Record_General(lu_name_, 'SPECNOTAXACCNT: Account :P1 is no tax account and not valid for posting type :P2', newrec_.code_part_value,newrec_.posting_type);
         END IF;
      ELSIF (ACCOUNTING_CODE_PART_A_API.Is_Stat_Account(newrec_.company, newrec_.code_part_value )) = 'TRUE' THEN
         Error_SYS.Record_General(lu_name_, 'SPECSTATACC: :P1 is a statistical account and not valid for posting type :P2', newrec_.code_part_value,newrec_.posting_type);
      END IF;
   ELSE
...
PROCEDURE Check_Insert___ (
   newrec_ IN OUT posting_ctrl_detail_spec_tab%ROWTYPE,
...
BEGIN
   ...
   Posting_Ctrl_Detail_API.Is_Spec_Control_Type_Exist(newrec_.company, newrec_.code_part, newrec_.posting_type, newrec_.pc_valid_from, newrec_.control_type_value, newrec_.valid_from, newrec_.spec_control_type);
   IF (newrec_.no_code_part_value = 'FALSE') THEN
      Error_SYS.Check_Not_Null(lu_name_, 'CODE_PART_VALUE', newrec_.code_part_value);
   END IF;

   IF (newrec_.spec_control_type = 'IC7') THEN
      -- Control type IC7, Income Type, needs currency code and country_code to get the internal id
      -- that should be saved.
      newrec_.spec_control_type_value := Income_Type_API.Get_Internal_Income_Type(newrec_.spec_control_type_value,
                                                                                  Company_Finance_API.Get_Currency_Code(newrec_.company),
                                                                                  Company_API.Get_Country_Db(newrec_.company));
   ELSIF (newrec_.spec_control_type = 'IC8') THEN
      -- Control type IC8, Irs1099_Type_Id, needs country_code to get the internal id to save
      $IF Component_Invoic_SYS.INSTALLED $THEN
         internal_id_ := Type1099_API.Get_Internal_Type1099(newrec_.spec_control_type_value, Company_API.Get_Country_Db(newrec_.company)); 
         newrec_.spec_control_type_value := internal_id_;
      $ELSE
         NULL;
      $END
   END IF;
   
   Posting_Ctrl_Detail_API.Validate_Control_Type_Value__ (newrec_.company, newrec_.spec_control_type_value,
                                                          newrec_.spec_control_type, newrec_.spec_module, newrec_.posting_type );
...
PROCEDURE Check_Update___ (
   oldrec_ IN     posting_ctrl_detail_spec_tab%ROWTYPE,
   newrec_ IN OUT posting_ctrl_detail_spec_tab%ROWTYPE,
   indrec_ IN OUT Indicator_Rec,
   attr_   IN OUT VARCHAR2 )
IS
...
BEGIN
   ...
   IF (newrec_.no_code_part_value = 'FALSE') THEN
      Error_SYS.Check_Not_Null(lu_name_, 'CODE_PART_VALUE', newrec_.code_part_value);
   END IF;
   Error_SYS.Check_Not_Null(lu_name_, 'CONTROL_TYPE_VALUE', newrec_.control_type_value);
...
PROCEDURE Check_Delete___ (
   remrec_ IN posting_ctrl_detail_spec_tab%ROWTYPE )
IS
BEGIN
   IF (App_Context_Sys.Find_Value(lu_name_||'.copy_to_company_', 'FALSE') != 'TRUE') THEN
...
PROCEDURE Copy_To_Companies__ (
   ...
   valid_from_              IN DATE,
   spec_control_type_value_ IN VARCHAR2,
   update_method_           IN VARCHAR2,
   log_id_                  IN NUMBER,
   attr_                    IN VARCHAR2 DEFAULT NULL)
IS
   source_rec_              posting_ctrl_detail_spec_tab%ROWTYPE;
   target_rec_              posting_ctrl_detail_spec_tab%ROWTYPE;
   old_target_rec_          posting_ctrl_detail_spec_tab%ROWTYPE;
   log_key_                 VARCHAR2(2000);
   log_detail_status_       VARCHAR2(10); 
BEGIN
   source_rec_ := Get_Object_By_Keys___(source_company_, code_part_, pc_valid_from_,posting_type_, control_type_value_, valid_from_, spec_control_type_value_);
   target_rec_ := source_rec_;
   target_rec_.company := target_company_;
   App_Context_Sys.Set_Value(lu_name_||'.copy_to_company_', 'TRUE');
   old_target_rec_ := Get_Object_By_Keys___(target_company_, code_part_,pc_valid_from_, posting_type_,control_type_value_, valid_from_, spec_control_type_value_);
   log_key_ := posting_type_||'^'||code_part_||'^'||pc_valid_from_||'^'||control_type_value_||'^'||valid_from_||'^'||spec_control_type_value_;
   ...
   ELSIF (source_rec_.rowversion IS NULL AND old_target_rec_.rowversion IS NULL) THEN
      ...
      Raise_Record_Not_Exist___(target_company_, code_part_, pc_valid_from_,posting_type_, control_type_value_, valid_from_, spec_control_type_value_);
...
IS
   ...
   spec_control_type_value_list_ VARCHAR2(32000);   
...
BEGIN
   ...
   WHILE (Client_SYS.Get_Next_From_Attr(attr_, ptr_, name_, value_)) LOOP
      ...
      ELSIF (name_ = 'SPEC_CONTROL_TYPE_VALUE_LIST') THEN
         spec_control_type_value_list_ := value_;         
...
PROCEDURE Copy_To_Companies_ (
   ...
   spec_control_type_value_list_ IN VARCHAR2,
   update_method_list_           IN VARCHAR2,
   copy_type_                    IN VARCHAR2,
   attr_                         IN VARCHAR2 DEFAULT NULL)
IS
   TYPE posting_ctrl_detail_spec IS TABLE OF posting_ctrl_detail_spec_tab%ROWTYPE INDEX BY BINARY_INTEGER;
   ...
   target_company_               posting_ctrl_detail_spec_tab.company%TYPE;
   ref_posting_ctrl_detail_spec_ posting_ctrl_detail_spec;
   ref_attr_                     attr;
   log_id_                       NUMBER;
...
BEGIN
   ...
   WHILE Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(value_, ptr_, code_part_list_) LOOP
      ref_posting_ctrl_detail_spec_(i_).code_part := value_;
      i_ := i_ + 1;
   END LOOP;
   ...
   WHILE Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(value_, ptr_, posting_type_list_) LOOP
      ref_posting_ctrl_detail_spec_(i_).posting_type := value_;
      i_ := i_ + 1; 
   END LOOP;
   ...
   WHILE Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(value_, ptr_, pc_valid_from_list_) LOOP
      ref_posting_ctrl_detail_spec_(i_).pc_valid_from := Client_SYS.Attr_Value_To_Date(value_);
      i_ := i_ + 1;
   ...
   WHILE Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(value_, ptr_, control_type_value_list_) LOOP
      ref_posting_ctrl_detail_spec_(i_).control_type_value := value_;
      i_ := i_ + 1;
   END LOOP;
   ...
   WHILE Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(value_, ptr_, valid_from_list_) LOOP
      ref_posting_ctrl_detail_spec_(i_).valid_from := Client_SYS.Attr_Value_To_Date(value_);
      i_ := i_ + 1;
   END LOOP;
   
   i_    := 0;
   ptr_  := NULL;
   WHILE Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(value_, ptr_, spec_control_type_value_list_) LOOP
      ref_posting_ctrl_detail_spec_(i_).spec_control_type_value := value_;
      i_ := i_ + 1;
   END LOOP;   
   ...
   WHILE Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(target_company_, ptr1_, target_company_list_) LOOP      
      IF (Copy_To_Company_Util_API.Get_Next_Record_Sep_Val(update_method_, ptr2_, update_method_list_)) THEN
         FOR j_ IN ref_posting_ctrl_detail_spec_.FIRST..ref_posting_ctrl_detail_spec_.LAST LOOP
            IF (ref_attr_.FIRST IS NOT NULL) THEN
               attr_value_ := ref_attr_(j_);
            END IF;            
            Copy_To_Companies__ (source_company_,  
                                 target_company_,
                                 ref_posting_ctrl_detail_spec_(j_).code_part,
                                 ref_posting_ctrl_detail_spec_(j_).pc_valid_from,
                                 ref_posting_ctrl_detail_spec_(j_).posting_type,
                                 ref_posting_ctrl_detail_spec_(j_).control_type_value,
                                 ref_posting_ctrl_detail_spec_(j_).valid_from,
                                 ref_posting_ctrl_detail_spec_(j_).spec_control_type_value,
                                 update_method_,
                                 log_id_,
                                 attr_value_);
         END LOOP;
...
PROCEDURE Insert_Posting_Ctrl_Det_Spec (
   company_                 IN VARCHAR2,
   posting_type_            IN VARCHAR2,
   code_part_               IN VARCHAR2,
   pc_valid_from_           IN DATE,
...
IS
   ...
   CURSOR get_posting IS
      ...
      AND    control_type_value = control_type_value_
      AND    valid_from = valid_from_
      AND    spec_control_type_value = spec_control_type_value_;
...
BEGIN
   ...
   IF (Company_Finance_API.Is_User_Authorized(company_ )) THEN
      ...
      ELSE
         ...
         newrec_.spec_control_type_value  := spec_control_type_value_;
         newrec_.spec_control_type        := spec_control_type_;
         newrec_.spec_module              := spec_module_;
      ...
      END IF;
   END IF;
END Insert_Posting_Ctrl_Det_Spec;
...
BEGIN
   ...
   IF (check_account%FOUND) THEN
      CLOSE check_account;
      Error_SYS.Record_General('PostingCtrlDetailSpec', 'ACCNTEXISTS: The account :P1 exists in posting control ', account_);
   END IF;
   CLOSE check_account;
END Account_Exist;
...
@SecurityCheck Company.UserAuthorized(company_)
PROCEDURE Remove_Posting_Ctrl_Det_Spec(
   company_                 IN VARCHAR2,
   code_part_               IN VARCHAR2,
   pc_valid_from_           IN DATE,
   posting_type_            IN VARCHAR2,
...
BEGIN
   Get_Id_Version_By_Keys___(objid_,
                             ...
                             control_type_value_,
                             valid_from_,
                             spec_control_type_value_);
   Remove__(dummy_info_,
            objid_,
            objversion_,
            'DO');
END Remove_Posting_Ctrl_Det_Spec;

@UncheckedAccess
FUNCTION Get_Crtl_Type_Value_Desc(
   company_                   IN VARCHAR2,
   spec_control_type_         IN VARCHAR2 ,
   spec_control_type_value_   IN VARCHAR2,
   spec_module_               IN VARCHAR2 ) RETURN VARCHAR2 
IS
   view_       VARCHAR2(100);
   pkg_name_   VARCHAR2(100);
   
   CURSOR get_information IS
       SELECT  view_name, pkg_name
       FROM   posting_ctrl_control_type_tab
       WHERE  control_type = spec_control_type_
       AND    module       = spec_module_;
BEGIN
   OPEN get_information;
   FETCH get_information INTO view_, pkg_name_;
   CLOSE get_information;
   RETURN Posting_Ctrl_Control_Type_API.Get_Control_Type_Desc_(company_ , 
                                                               spec_control_type_ , 
                                                               spec_control_type_value_ ,
                                                               view_ ,
                                                               pkg_name_,
                                                               spec_module_ );
END Get_Crtl_Type_Value_Desc;
