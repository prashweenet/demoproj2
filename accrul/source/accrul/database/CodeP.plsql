...
BEGIN      
   super(attr_);
   Accounting_Code_Part_Value_API.Add_Default_Values_Additional(attr_, 'P');
...
BEGIN
   IF ((INSTR( newrec_.code_part_value,'&') > 0) OR (INSTR( newrec_.code_part_value,'^') > 0) OR (INSTR( newrec_.code_part_value,'''') > 0)) THEN
      Error_SYS.Record_General('CodeP', 'INVCHARACTORS: You have entered an invalid character in this field');
   END IF;
