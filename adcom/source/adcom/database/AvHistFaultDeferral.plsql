...
--  Date    Sign    History
--  ------  ------  ---------------------------------------------------------
--  200915  themlk  DISCOP2020R1-123, New, Modify, Get_Objects_By_Keys, Get_Fault_Deferral_Id
...
PROCEDURE Check_Insert___ (
   newrec_ IN OUT av_hist_fault_deferral_tab%ROWTYPE,
...
IS
   ...
   SELECT av_hist_fault_def_seq.nextval 
   FROM dual;
BEGIN
   ...
   IF(newrec_.fault_deffer_id IS NULL) THEN
      newrec_.fault_deffer_id := key_id_;
   END IF;
   Client_SYS.Add_To_Attr('FAULT_DEFFER_ID', key_id_, attr_);
...
FUNCTION Get_Fault_Deferral_Id(
   mx_unique_key_ IN VARCHAR2) RETURN NUMBER
IS
   CURSOR get_fault_deferrel_id IS
   SELECT fault_deffer_id
   FROM av_hist_fault_deferral_tab
   WHERE mx_unique_key = mx_unique_key_;
   
   fault_deffer_id_ av_hist_fault_deferral_tab.fault_deffer_id%TYPE;
BEGIN
   IF (mx_unique_key_ IS NULL) THEN
      RETURN NULL;
   END IF;
   OPEN get_fault_deferrel_id;
   FETCH get_fault_deferrel_id INTO fault_deffer_id_;
   CLOSE get_fault_deferrel_id;
   RETURN fault_deffer_id_;
EXCEPTION
   WHEN no_data_found THEN
      RETURN NULL;  
END Get_Fault_Deferral_Id;

PROCEDURE New (
   newrec_ IN OUT NOCOPY av_hist_fault_deferral_tab%ROWTYPE )
IS
   BEGIN
   New___(newrec_);
   END New;
...
FUNCTION Get_Object_By_Keys (
   fault_deffer_id_ IN NUMBER ) RETURN av_hist_fault_deferral_tab%ROWTYPE
IS
BEGIN
   RETURN Get_Object_By_Keys___(fault_deffer_id_);
END Get_Object_By_Keys;


FUNCTION Mig_Get_Fault_Deffer_Id(
   mx_unique_key_ IN VARCHAR2) RETURN NUMBER
IS
...
BEGIN
   ...
   IF fault_deffer_id_ IS NULL THEN
      fault_deffer_id_ := av_hist_fault_def_seq.nextval;
   END IF;
   
   RETURN fault_deffer_id_;   
   
END Mig_Get_Fault_Deffer_Id;
...
IS
   ...
   fault_barcode_              av_hist_fault_deferral_tab.fault_barcode%TYPE;
