-----------------------------------------------------------------------------
--
--  Logical unit: AvAssemblyFlightMeas
--  Component:    ADCOM
--
...
IS  
   CURSOR get_key IS
      ...
      FROM  av_assembly_flight_meas_tab
      WHERE assembly_type_code = CLIENT_SYS.Get_Item_Value('ASSEMBLY_TYPE_CODE', attr_) AND 
            measurement_type_code = CLIENT_SYS.Get_Item_Value('MEASUREMENT_TYPE_CODE', attr_);
BEGIN
   OPEN  get_key;
